			...
			for (songNotes in section.sectionNotes)
			{
				...
				if(floorSus > 0) {
					...
					for (susNote in 0...floorSus)
					{
						...
						var sustainNote:Note = new Note(daStrumTime + (Conductor.stepCrochet * susNote) + (Conductor.stepCrochet / 2 / SONG.speed), daNoteData, oldNote, true);
						endNote = sustainNote;
						...
						//sustainNote.offsetY = swagNote.height / 2;
					...
					if(endNote != null) {
						endNote.scale.y = endNote.scale.x;
						endNote.baseScaleY = endNote.scale.y;
